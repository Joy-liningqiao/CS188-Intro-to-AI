...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        "*** YOUR CODE HERE ***"
        manhattan = lambda x1, x2, y1, y2: min(abs(x1-x2),abs(y1-y2))
        ...
        if sum(newScaredTimes)==0:
            ...
            return 5*len(foodList)-max(newScaredTimes)+c
...
def betterEvaluationFunction(currentGameState):
    ...
    DESCRIPTION: Priority to collect food pellets and avoid ghosts when scaredTimes of ghosts =0, else priority is to go towards ghosts
    """
    "*** YOUR CODE HERE ***"
    from statistics import mean
