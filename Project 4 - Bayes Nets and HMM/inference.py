...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        if noisyDistance==None and (ghostPosition==jailPosition): 
            ...
            noisyGivenTrue=busters.getObservationProbability(noisyDistance, trueDistance)
    ...
    def elapseTime(self, gameState):
        ...
        for pos in positions:
            ...
            for newPos, prob in newPosDist.items():
                beliefDict[newPos]=beliefDict[newPos]+self.beliefs[pos]*prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        frac=self.numParticles//len(positions)
        while len(self.particles) <= self.numParticles:
            for pos in positions:
                repeat=[pos]*frac
                self.particles.extend(repeat)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        positions=self.legalPositions
        cartProd=[i for i in itertools.product(positions, repeat=self.numGhosts)]
        while len(self.particles)<self.numParticles:
            self.particles.append(random.choice(cartProd))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        resample=[]
        pacmanPosition=gameState.getPacmanPosition()
        for comb in self.particles:
            obs=[]
            for i in range(self.numGhosts):
                probObs=self.getObservationProb(observation[i], pacmanPosition, comb[i], self.getJailPosition(i))
                obs.append(probObs)
            weights[comb]+=prod(obs)
        if weights.total()==0:
            self.initializeUniformly(gameState)
        else:
            weights.normalize()
